...
class StateEncoder(nn.Module):
    def __init__(self, config: StateEncoderConfig) -> None:
        super().__init__()
        self.config = config
        ch_list = [config.state_dim] + config.hidden_sizes + [config.z_channels]
        
        length = len(ch_list) - 1
        self.layers = nn.ModuleList()
        # import pdb
        # pdb.set_trace()
        for i, (ch_in, ch_out) in enumerate(zip(ch_list[:-1], ch_list[1:])):
            if i != length - 1:
    ...
    def forward(self, x): # x: (B, T, N, Obs_dim)
        ...
        #### TODO: 如果训练没有效果，就尝试开启这个LayerNorm
        # x = self.embed_ln(x)
        return x
